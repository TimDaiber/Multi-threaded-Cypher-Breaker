...
public class RailFence {
	...
	public String decrypt(String encryptedText, int key) {
		// Declare a 2D array of key rows and text length columns
		char[][] matrix = new char[key][encryptedText.length()]; // The array is
																	// filled
																	// with
																	// chars
																	// with
		...
		do {
			int row = 0; // Used to keep track of rows
			boolean down = true; // Used to zigzag
			for (int i = 0; i < encryptedText.length(); i++) { // Loop over the
																// plaintext
				if (row == targetRow) {
					matrix[row][i] = encryptedText.charAt(index); // Add the
																	// next
																	// character
																	// in the
																	// plaintext
